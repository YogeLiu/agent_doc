...
- $\widehat{D}(\mathbf{q}, \mathbf{x})$：查询向量 $\mathbf{q}$ 与库向量 $\mathbf{x}$ 之间的**近似距离**
- $m$：子空间数量（PQ 将向量切分成 $m$ 段）
- $j$：第 $j$ 个子空间的索引（$j=1,\ldots,m$）
- $q^{(j)}(\mathbf{x})$：库向量 $\mathbf{x}$ 在第 $j$ 个子空间的 PQ code（即量化后的 centroid 索引，取值范围 $0 \sim k-1$）
- $T[j][q^{(j)}(\mathbf{x})]$：从预计算的距离表中**查表**，获取查询向量 $\mathbf{q}$ 的第 $j$ 段子向量与库向量 $\mathbf{x}$ 在第 $j$ 段对应 centroid 之间的距离
- $\sum_{j=1}^{m}$：将 $m$ 个子空间的距离累加，得到总的近似距离

**直观理解**：

对于库向量 $\mathbf{x}$，我们已经把它编码成了 $m$ 个整数 $(q^{(1)}(\mathbf{x}), q^{(2)}(\mathbf{x}), \ldots, q^{(m)}(\mathbf{x}))$。计算距离时，不需要访问原始浮点向量，只需：

1. 用每个整数作为索引
2. 在对应子空间的距离表中查找预计算好的距离值
3. 把 $m$ 个距离值加起来

这样每个候选向量只需 $m$ 次内存访问（查表）+ $m$ 次加法，而不是 $d$ 次浮点乘法和加法，实现了巨大的加速。
